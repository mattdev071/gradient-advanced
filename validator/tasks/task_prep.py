...
async def prepare_text_task(task: AnyTextTypeRawTask, keypair: Keypair) -> tuple[str, str, str]:
    ...
    if not task.test_data:
        ...
        if has_multiple_datasets:
            ...
            # Clean merged samples
            merged_samples = clean_text_dataset(merged_samples)
            ...
            dataset = dataset.filter(lambda x: all(v is not None and (not isinstance(v, str) or v.strip()) for v in x.values()))
...
def clean_text_dataset(dataset):
    # Remove duplicates and empty/corrupt samples
    seen = set()
    cleaned = []
    for row in dataset:
