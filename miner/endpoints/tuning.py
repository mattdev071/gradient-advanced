...
async def task_offer(
    ...
    try:
        ...
        # --- STRATEGY: Accept only high-value, high-weight, or tournament jobs ---
        # Accept if job is part of a tournament (tournament_id present)
        # Accept if job has high hours_to_complete, large dataset, or large model size
        # Otherwise, reject

        # Tournament detection (if tournament_id is present in request)
        tournament_id = getattr(request, 'tournament_id', None)
        if tournament_id:
            logger.info(f"Accepting offer: Tournament job detected (tournament_id={tournament_id})")
            return MinerTaskResponse(message="Accepted: Tournament job", accepted=True)

        # Accept only text tasks of supported types
        ...
        # Thresholds for high-value jobs
        min_hours = 1
        min_dataset_size = 100_000
        min_model_params = 3_000_000_000  # 7B params

        # Use ds_size if present, else fallback to 0
        dataset_size = getattr(request, 'ds_size', 0) or 0
        model_params = getattr(request, 'model_params_count', 0) or 0

        # Accept if job is large enough and not currently busy
        if (current_job_finish_time is None or current_time + timedelta(hours=1) > current_job_finish_time):
            if (
                request.hours_to_complete >= min_hours or
                dataset_size >= min_dataset_size or
                model_params >= min_model_params
            ):
                logger.info(f"Accepting offer: High-value job (hours={request.hours_to_complete}, ds_size={dataset_size}, model_params={model_params})")
                return MinerTaskResponse(message="Accepted: High-value job", accepted=True)
            else:
                logger.info(f"Rejecting offer: Job too small (hours={request.hours_to_complete}, ds_size={dataset_size}, model_params={model_params})")
                return MinerTaskResponse(message="Rejected: Only accept high-value, high-weight, or tournament jobs", accepted=False)
        else:
            return MinerTaskResponse(
